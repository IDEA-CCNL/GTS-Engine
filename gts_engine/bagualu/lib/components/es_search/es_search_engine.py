...
from gts_engine.bagualu.lib.framework.classification_finetune.consts import (
    Label2Token,
    LabeledSample
)
from gts_engine.bagualu.lib.framework.classification_finetune import (
    DataReaderClf
)
...
class EsSearchArgsProto(Protocol):
    """es search参数类型协议"""
...
class ESSearchEngine(OptionalLoggerMixin):
    ...
    def __init__(
        ...
        Args:
            args (EsSearchArgsProto):
                引擎参数，定义在`.es_search_args.ESSearchArgs`中
            label2token (Label2Token):
                样本对应的label2token
        """
    ...
    def es_search_augment(self) -> None:
        ...
        通过参数中的训练样本路径，计算查询向量并访问ES Search服务进行数据增强，并写入缓存文件。

        Raises:
            Exception: 增强数据缓存文件路径augm_es_path为None
        """
        # 执行条件
        ...
        self._args.es_search_args.es_search_num = self.__cal_es_search_num(
            search_text_list)
        if (len(search_text_list) == 0 or
                self._args.es_search_args.es_search_num == 0):
            self.info("no text needs to be augmented, skip es search")
            return
        self.info(f"length of the text list for es search(`len("
                  f"search_text_list)`): {len(search_text_list)}")
        self.info(f"search number for each text(`es_search_args.es_search_num`"
                  f")：{self._args.es_search_args.es_search_num}")
        # 生成搜索向量
        cuda_id = f"cuda:{self._args.gpu_id}"
        device = torch.device(cuda_id if torch.cuda.is_available() else "cpu")
        model = self.__load_sbert_client(device)
        query_vector_list = self.__gen_query_vector_list(
            search_text_list, model, device)
        ...
        client = search_client_factory(
            self._args.es_search_args.es_cllient)(self._args)
        es_search_result_num = 0
        with open(
            self._args.es_search_args.augm_es_path, 'w', encoding='utf8'
        ) as output_file, mp.Pool(
            processes=self._args.es_search_args.es_num_thread,
            ...
            for searched_sample_list in tqdm(iters, desc='es Augmentation'):
                if searched_sample_list is not None:
                    output_file.writelines(
                        [json.dumps(asdict(searched_sample),
                                    ensure_ascii=False) + "\n"
                         for searched_sample in searched_sample_list])
                    es_search_result_num += len(searched_sample_list)
        es_end_time = time.time()
        # 输出信息
        self.info('the spend time of es search :%d' % (
            es_end_time - self._args.es_search_args.start_time))
    ...
    def __load_text_dict(self) -> Dict[str, List[Id_Text]]:
        ...
        if self._args.train_data_path is not None and os.path.exists(
                self._args.train_data_path):
            if self._args.es_search_args.add_label_description:
                sample_handler: Callable[[LabeledSample], Id_Text] = \
                    lambda sample: (sample.id, sample.text +
                                    ',这是' + sample.label)
            else:
                sample_handler: Callable[[LabeledSample], Id_Text] = \
                    lambda sample: (sample.id, sample.text)
            for sample in DataReaderClf.read_labeled_sample(
                    self._args.train_data_path, self._label2token):
                if sample.label not in text_dict.keys():
                    text_dict[sample.label] = []
                text_dict[sample.label].append(sample_handler(sample))
        if self._args.unlabeld_data_path is not None and os.path.exists(
                self._args.unlabeld_data_path):
            text_dict["unlabeled"] = [
                (sample.id, sample.text) for sample in
                DataReaderClf.read_unlabeled_sample(
                    self._args.unlabeld_data_path)]
        return text_dict

    def __gen_search_text_list(
        self,
        text_dict: Dict[str, List[Id_Text]]
    ) -> List[Id_Text]:
        """根据条件加载需要进行搜索的句子列表
        ...
        # 如果训练文本数量足够，则无需增强
        if (text_num > self._args.es_search_args.max_augment_data or
                text_num == 0):
            pass
        # 如果训练文本数量不足但超出搜索query上限，需要筛选用于搜索的text
        elif (self._args.es_search_args.stratification_sample and
                text_num > self._args.es_search_args.es_search_query):
            ...
            search_num_each_label = (
                int(self._args.es_search_args.es_search_query / label_num) if
                label_num > 0 else 0)
            # 优先使用标注数据
            ...
            # 多余的query用无标注数据填充
            unlabeled_search_num = (self._args.es_search_args.es_search_query -
                                    len(search_text_list))
    ...
    def __cal_es_search_num(self, search_text_list: List[Id_Text]) -> int:
        """计算每条text需要搜索的结果数量"""
        es_search_num = ((
            int(self._args.es_search_args.max_augment_data /
                len(search_text_list)) +
            self._args.es_search_args.addtion_search) if
            len(search_text_list) > 0 else 0)
    ...
    def __gen_query_vector_list(
        ...
        for idx in range(0, len(search_text_list), batch_size):
            ...
            query_vector_batch: List[torch.Tensor] = model.encode(
                text_batch, device=device)  # type: ignore
            id_query_vector_list += [
                (id_text_batch[i][0], query_vector_batch[i])
                for i in range(len(id_text_batch))]
        return [(id_query_vector[0], id_query_vector[1].tolist())
                for id_query_vector in id_query_vector_list]
