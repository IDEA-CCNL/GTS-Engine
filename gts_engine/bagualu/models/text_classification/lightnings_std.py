...
class TrainLightningClfStd(BaseTrainingLightningClf):
    ...
    def training_step_end(self, step_output: Dict[str, Tensor]):
        ...
        # avg = lambda loss_list: (loss_list * (batch_size_list / batch_size_list
        #                                       .sum())).sum()
        loss = self.avg(step_output["loss"], batch_size_list)
        loss_mlm = self.avg(step_output["loss_mlm"], batch_size_list)
        loss_ce = self.avg(step_output["loss_ce"], batch_size_list)
        self._logger.info(
    ...
    def validation_step_end(self, validation_step_outputs: Dict[str, Tensor]):
        ...
        step_loss = (loss_list * (batch_size_list / batch_size_list.sum())
                     ).sum()  # loss按batch_size加权平均
