...
class BagualuSummaryLitModel(LightningModule):
    ...
    def __init__(self,
        ...
        self._model = BagualuSummaryModel(args.pretrained_model_root, args.max_dec_length)
        self._tokenizer = PegasusTokenizer.from_pretrained(self.args.pretrained_model_root)
        ...
        self._rouge_metric = ROUGEScore(
            rouge_keys=rouge_keys, normalizer=lambda x: x)
    ...
    def training_step(self, batch: dict, batch_idx: int):  # pylint: disable=signature-differs,arguments-differ,unused-argument
        ...
        if self.global_rank == 0:
             self._logger.info(f"loss: {loss:.4f} lr: {lr:.2e} epoch: {self.current_epoch} step: {self.global_step}")
    ...
    def validation_epoch_end(self, outputs) -> None:
        ...
        for k, v in rouge_dict.items():
            metrics['dev_{}'.format(k)] = v
       
        metrics['dev_loss'] = dev_loss
            
        self.log_dict(metrics, prog_bar=False, logger=True, sync_dist=True)

        rougeL = rouge_dict['rougeL_fmeasure'] if 'rougeL_fmeasure' in rouge_dict else 0.
        rouge1 = rouge_dict['rouge1_fmeasure'] if 'rouge1_fmeasure' in rouge_dict else 0.
        rouge2 = rouge_dict['rouge2_fmeasure'] if 'rouge2_fmeasure' in rouge_dict else 0.


        if self.global_rank == 0:
            self._logger.info(f"validation - loss: {dev_loss:.4f} rougeL_fmeasure: {rougeL:.4f}, rouge1_fmeasure: {rouge1:.4f}, rouge2_fmeasure: {rouge2:.4f} epoch: {self.current_epoch}")
    ...
    def predict_step(self, batch: dict, batch_idx: int) ->  List[Dict]: 
        ...
        preds = self._tokenizer.batch_decode(
            model_output['generated_ids'], skip_special_tokens=True, clean_up_tokenization_spaces=True)
