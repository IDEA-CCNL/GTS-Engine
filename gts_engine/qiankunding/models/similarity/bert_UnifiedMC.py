...
from transformers import (Adafactor, AutoConfig, AutoModelForMaskedLM,
...
class taskModel(nn.Module):
    ...
    def __init__(self, pre_train_dir: str, tokenizer):
        ...
        if "1.3B" in pre_train_dir:
            # v100
            try:
                logger.info("gpu_type {}".format(
                    globalvar.get_value("gpu_type")))
    ...
    def forward(self,
        ...
        cls_logits = mlm_logits[:, :, self.yes_token].view(
            -1, seq_len) + clslabels_mask
        hidden_states = outputs.hidden_states[-1]

        if mlmlabels is None:
...
class BertUnifiedMCForMatch(BaseModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        mask_acc = self.compute_metrix(logits, batch['mlmlabels'],
                                       batch['mlmlabels_mask'])
        cls_acc, ncorrect, ntotal = self.compute_metrix(
            cls_logits, batch['clslabels'])
    ...
    def predict(self, batch):
        ...
        predicts_ = []
        labels_ = []
        for label_idx, prob, predict, label in zip(batch["label_idx"], probs,
                                                   predicts, labels):
            ...
            for i in range(batch_size):
                ...
                if sep_token_indexes[0] + 1 < sep_token_indexes[1]:
                    sample_hidden_states = hidden_states[
                        i, sep_token_indexes[0] + 1:sep_token_indexes[1], :]
    ...
    def compute_metrix(self, logits, labels, mlmlabels_mask=None):
        ...
        if is_mlm:
            ...
            return torch.sum(corr.float()) / labels.size(0), torch.sum(
                corr.float()), labels.size(0)
