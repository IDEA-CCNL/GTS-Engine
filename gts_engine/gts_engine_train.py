...
from qiankunding.dataloaders.nli.dataloader_UnifiedMC import \
    TaskDataModelUnifiedMCForNLI
from qiankunding.models.nli.bert_UnifiedMC import BertUnifiedMCForNLI

globalvar._init()
from gts_common.logs_utils import Logger
from gts_common.utils.detect_gpu_memory import decide_gpu, detect_gpu_memory
...
def train(args):
    ...
    train_pipeline = PIPELINE_REGISTRY.get(name="train_pipeline",
                                           suffix=train_pipeline_module)
    train_pipeline(args)
...
def main():
    ...
    total_parser.add_argument("--engine_type",
                              ...
                              type=str,
                              help="engine type")
    total_parser.add_argument("--train_mode",
                              required=True,
    ...
    total_parser.add_argument('--num_workers',
                              default=8,
                              type=int,
                              help="number of workers for data preprocessing.")
    total_parser.add_argument('--train_batchsize',
                              default=1,
                              type=int,
                              help="batch size of train dataset")
    total_parser.add_argument('--valid_batchsize',
                              default=4,
                              type=int,
                              help="batch size of validation dataset")
    total_parser.add_argument('--test_batchsize',
                              default=4,
                              type=int,
                              help="batch size of test dataset")
    total_parser.add_argument('--max_len',
                              default=512,
                              type=int,
    ...
    total_parser.add_argument(
        ...
        required=True,
        type=str,
        help="directory which contains all downloaded pretrained models")
    total_parser.add_argument('--data_dir',
                              required=True,
                              type=str,
                              help="data directory of specific task data")
    total_parser.add_argument('--train_data',
                              required=True,
                              type=str,
                              help="filename of train dataset")
    total_parser.add_argument('--valid_data',
                              required=True,
                              type=str,
                              help="filename of validation dataset")
    total_parser.add_argument('--test_data',
                              default='test.json',
                              type=str,
                              help="filename of test dataset")
    total_parser.add_argument('--label_data',
                              default='labels.json',
                              type=str,
                              help="filename of label data")
    total_parser.add_argument(
        '--unlabeled_data',
        default='test.json',
        type=str,
        help="filename of unlabeled data")  # unlabeled.json
    total_parser.add_argument(
        '--save_path',
        default='output',
        type=str,
        help="save path for trained model and other logs")
    total_parser.add_argument('--threshold',
                              default=0.8,
                              type=float,
                              help="pseudo threshold")

    # * Args for general setting
    total_parser.add_argument('--seed',
                              default=1234,
                              type=int,
                              help="random seed for training")
    total_parser.add_argument('--lr',
                              default=2e-5,
                              type=float,
                              help="learning rate")

    # * Args for Trainer
    total_parser.add_argument('--max_epochs',
                              default=None,
                              type=int,
                              help="upper limit of training epochs")
    total_parser.add_argument('--min_epochs',
                              default=None,
                              type=int,
                              help="lower limit of training epochs")
    total_parser.add_argument(
        '--val_check_interval',
        default=0.5,
        type=float,
        help="perform a validation loop every after every `N` training epochs")

    print("total_parser:", total_parser)
    args = total_parser.parse_args(namespace=GtsEngineArgs())

    logger.info(f"pretrained_model_dir {args.pretrained_model_dir}")
    args.gpus = 1
