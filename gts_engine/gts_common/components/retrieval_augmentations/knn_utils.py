    ...
    def enhance(self, label_embeds_dict):
        ...
        for label, label_embeds in label_embeds_dict.items():
            ...
            for c_embeds in itertools.combinations(label_embeds,
                ...
                enhance_num += 1
        logger.info(f"enhance num: {enhance_num}")
...
class BasicDatastore:
    ...
    def construct(self, classify_model, data_model, dataset_name):
        ...
        for batch in tqdm(data_loader):
            ...
            logits, inner_probs, _, labels, sample_embeds = classify_model.predict(
                batch)
            if dataset_name != "unlabeled":
        ...
        for label, enhanced_embeds in label_enhanced_embeds_dict.items():
            ...
            probs.append(
                np.eye(inner_probs.shape[1])[[label] *
                                             enhanced_embeds.shape[0]])
        ...
        logger.info("datastore{} embed size: {}".format(
            dataset_name, embeds.shape))
...
def get_datastores(dataset_names, model, data_model):
    ...
    for dataset_name in dataset_names:
        ...
        datastore.construct(classify_model=model,
...
def get_classify_info(model, data_loader):
    ...
    logger.info(f"model classify prob shape: {classify_probs.shape}")
    logger.info(f"sample embed original shape: {sample_embeds.shape}")
...
def get_knn_probs(distances, datastore_probs, k, remove_self=False):
    ...
    if remove_self:
        ...
        invalid_row_indices = np.where(
            np.count_nonzero(valid_distances, axis=1) != k - 1)[0]  # 存在问题的行
        for invalid_row_index in invalid_row_indices:
            invalid_col_indices = np.where(
                valid_distances[invalid_row_index] is False)[0]  # 存在问题的列
            for invalid_col_index in invalid_col_indices[1:]:
                valid_distances[invalid_row_index][
                    invalid_col_index] = True  # 只保留一个
        nearest_indices = nearest_indices[valid_distances].reshape(
            distances.shape[0], k - 1)
        nearest_distances = nearest_distances[valid_distances].reshape(
            distances.shape[0], k - 1)
...
def grid_search_for_hyper(model, data_model, datastores):
    """超参搜索."""
    y_true, _, classify_probs, sample_embeds = get_classify_info(
        model, data_model.val_dataloader())
    ...
    for hyper_tuple in itertools.product(*lambda_search_list, *k_search_list):
        ...
        for index in range(len(datastores)):
            ...
            if k_values[index] in knn_cache[index]:
                ...
                knn_probs = get_knn_probs(distance_cache[index],
                                          datastore_probs, k_values[index])
        ...
        if current_acc > best_acc:
            ...
            logger.info("best acc: {}  best hyper: {}".format(
                round(best_acc * 100, 2), best_hyper))
...
def knn_augmentation(model, data_model, output_dir):
    ...
    best_hyper, dev_y_true, dev_best_y_pred = grid_search_for_hyper(
        model, data_model, datastores)
    return best_hyper, datastores


def knn_inference(sample_embeds, knn_datastores, knn_best_hyper):
    total_knn_probs = None
    for l, k, datastore in zip(knn_best_hyper["lambda_values"],
                               knn_best_hyper["k_values"], knn_datastores):
        if l <= 1e-5 or k <= 0:
            continue
        remove_self = True if datastore.get_dataset_name(
        ) == "unlabeled" else False  # 检索时是否移除自身
        if remove_self:
        ...
        distances = pairwise_distances(query_embeds,
                                       ...
                                       metric="euclidean",
                                       n_jobs=1)
        knn_probs = get_knn_probs(distances,
                                  datastore_probs,
                                  k,
