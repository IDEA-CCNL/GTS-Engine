...
class BaseAdaptiveValidationInterval(
    Callback,
    OptionalLoggerMixin,
    metaclass=ABCMeta
):
...
class AdaptiveValIntervalFixed(BaseAdaptiveValidationInterval):
    ...
    def __init__(
        ...
        self.__val_interval = max(
            1, batch_num_per_epoch // val_times_per_epoch)
...
class AdaptiveValIntervalDevLoss(BaseAdaptiveValidationInterval):
    ...
    def __init__(
        ...
        self.__batch_num_each_epoch = (
            self.__train_sample_num // self.__batch_size)
        self.__val_interval = max(
    ...
    def __loss_rising_flag(self, window=2, pre_sample_len=4):
        ...
        """
        dev_sample_start = (
            len(self.__dev_loss_list) - window if
        ...
        pre_sample_end = (
            pre_sample_start + window if
            pre_sample_start + window <= len(self.__dev_loss_list) else
            len(self.__dev_loss_list))
        pre_step_loss_avg = np.mean(
            self.__dev_loss_list[pre_sample_start:pre_sample_end])
        dev_step_loss_avg = np.mean(
            self.__dev_loss_list[dev_sample_start:])
...
class AdaptiveValIntervalTrainLoss(BaseAdaptiveValidationInterval):
    ...
    def __init__(
        ...
        self.__sample_gap = max(
            min(
                15 // max(int((batch_size // 8) ** 0.5), 1),
                self.__batch_num_per_epoch // 8
            ),
            1
        )
    ...
    def _get_validation_flag(
        ...
        return (self.__dev_gate and
                trainer.global_step % self.__val_interval == 0)
