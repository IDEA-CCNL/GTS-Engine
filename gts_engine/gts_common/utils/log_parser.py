...
class LogParser(metaclass=ABCMeta):
    ...
    只需继承和实现 `_target_info_dict` 属性，将指标和对应的提取函数注册到该字典中，
    便可以通过 `parse_log()` 方法解析日志，得到指标字典。具体用法可以参照下面两个子类的实现。
    """
...
class StudentLogParser(LogParser):
    ...
    def __dev_acc_matcher(self, log: str) -> Optional[str]:
        ...
        if len(multi_model_dev_acc_list) > 0:
            ...
            return re.findall(
                r"(?<=save checkpoint. dev acc: )0\.\d+", log)[-1]

    def __train_test_acc_matcher(self, log: str) -> Optional[str]:
        return re.findall(
            r"(?<=INFO: training model test acc: )0\.\d+", log)[0]

    def __inf_test_acc_matcher(self, log: str) -> Optional[str]:
        return re.findall(
            r"(?<=INFO: inference model test acc: )0\.\d+", log)[0]
